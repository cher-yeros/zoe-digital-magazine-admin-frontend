          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                {article.current_revision?.categories &&
                  article.current_revision.categories.length > 0 && (
                    <>
                      <span>•</span>
                      <span>
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={() =>
                  handleReview(article.id, article.current_revision?.id || "")
                }
              >
              ...
              {article.current_revision?.status === "submitted" && (
                <>
                  <Button
                    ...
                    onClick={() =>
                      handleQuickApprove(article.current_revision?.id || "")
                    }
                  >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleQuickReject(article.current_revision?.id || "")
                    }
                  >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="submitted">
            Submitted
            {submissions.filter(
              (s) => s.current_revision?.status === "submitted"
            ).length > 0 && (
          ...
          <TabsTrigger value="in_review">
            In Review
            {submissions.filter(
              (s) => s.current_revision?.status === "in_review"
            ).length > 0 && (
              <Badge variant="secondary" className="ml-2">
                {
                  submissions.filter(
